    ...
    while not DFSStack.isEmpty():
        ...
        for Child in problem.getSuccessors(CurrentNode[0]):
            ...
            if  Child[0] not in visited:#not yet visit
                ...
                DFSStack.push((Child[0],tempPath))
        ...
        for Child in problem.getSuccessors(CurrentNode[0]):
            ...
            if  Child[0] not in visited:
                if(problem.isGoalState(Child[0]) == False):
                    visited.add(Child[0])
                tempPath.append(Child[1])
                BFSQueue.push((Child[0],tempPath))   
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    visited=set()
    visited.add(startstate)
    ASPriorityQueue = util.PriorityQueue()
    ASPriorityQueue.push((startstate,[]),0)
   
    
    while not ASPriorityQueue.isEmpty():
        CurrentNode=ASPriorityQueue.pop()
        path = CurrentNode[1]
        ...
        for Child in problem.getSuccessors(CurrentNode[0]):
            ...
            if Child[0] not in visited:
                if(problem.isGoalState(Child[0]) == False):
                    visited.add(Child[0])
                tempPath.append(Child[1])
                ASPriorityQueue.push((Child[0],tempPath),problem.getCostOfActions(tempPath) + heuristic(Child[0],problem)) ########## f(n) = g(n) + h(n)
    return path
    util.raiseNotDefined()
